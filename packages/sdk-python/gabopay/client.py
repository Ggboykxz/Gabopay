...
class Gabopay:
    ...
    def charges_create(self, options: ChargeCreateOptions) -> Charge:
        """Create a new charge."""
        return self._request("POST", "/v1/charges", options)

    def charges_get(self, charge_id: str) -> Charge:
        """Get a charge by ID."""
        return self._request("GET", f"/v1/charges/{charge_id}")
    ...
    def refunds_create(self, transaction_id: str, options: RefundCreateOptions) -> Refund:
        """Create a refund."""
        return self._request("POST", f"/v1/refunds/{transaction_id}", options)

    def refunds_get(self, refund_id: str) -> Refund:
        """Get a refund by ID."""
        return self._request("GET", f"/v1/refunds/{refund_id}")

    def payouts_create(self, options: PayoutCreateOptions) -> Payout:
        """Create a payout."""
        return self._request("POST", "/v1/payouts", options)
